...
const Products = () => {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    return [product.title, product.body]
      .join(' ')
      .toLowerCase()
      .includes(query)
  })
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="max-w-xl">
            ...
            <input
              ...
              placeholder="Buscar por título o body"
                  ...
                  <tbody className="divide-y divide-slate-200 bg-white">
                    {currentProducts.map((product) => (
                      <tr key={product.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-6 py-4 align-top text-sm text-slate-600 max-w-2xl wrap-break-word">{product.body}</td>
                        <td className="px-6 py-4 align-top text-sm text-slate-700">
                          <span className="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">{product.userId}</span>
                        </td>
                        <td className="px-6 py-4 align-top text-sm text-slate-700">
